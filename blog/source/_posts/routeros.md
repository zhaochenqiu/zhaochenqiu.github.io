...
这样网卡1的ip地址就被设置为10.250.95.40。这里注意，**如果有静态ip一定要把两块网卡的ip对应设置，千万不能弄反了**。
接着配置路由转发规则,这里为了简单，直接全网转发，输入命令：
`ip route add dst-address=0.0.0.0/24 gateway=222.198.156.1`
设置好了以后使用winbox就可以连接，或者直接在浏览器中输入地址也可以访问。否则就无法连接。
...
![routeros6.png](http://i3.tietuku.com/22307875799597b2.png)
